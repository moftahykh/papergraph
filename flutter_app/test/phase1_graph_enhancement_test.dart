...
void main() {
  group('Phase 1: Graph Enhancement & Anti-AI-Slop Tests', () {
    test('GraphCanvasPainter shouldRepaint returns false when state is unchanged', () {
      const nodes = [
        GraphNode(
          id: '1',
          canonicalId: '1',
    ...
    testWidgets('ConnectedGraphView includes dedicated zoom in and zoom out buttons and InteractiveViewer', (tester) async {
      final snapshot = GraphSnapshot(
        graphId: 'test-graph',
        origin: const GraphOrigin(id: 'o1', canonicalId: 'o1', title: 'Seed Paper', year: 2023),
        status: GraphJobStatus.completed,
        createdAt: DateTime.now(),
        nodes: const [
          GraphNode(id: 'o1', canonicalId: 'o1', title: 'Seed Paper', year: 2023, citationCount: 10, isOrigin: true),
        ],
        citationEdges: const [],
        similarityEdges: const [],
      );
    ...
    test('HiveService multi-key indexing allows 0-second offline graph retrieval by DOI, CanonicalId, URL, or Title', () async {
      final tempDir = await Directory.systemTemp.createTemp('hive_graph_offline_test_');
      ...
      expect(byDoi!.origin.title, equals('Quantum Teleportation Across 143 km'));

      // 3. Retrieve by prefixed DOI 'doi:10.1038/nature12373'
      final byPrefixedDoi = HiveService.getCachedGraph('doi:10.1038/nature12373');
      expect(byPrefixedDoi, isNotNull);

      // 4. Retrieve by full URL 'https://doi.org/10.1038/nature12373'
      final byUrl = HiveService.getCachedGraph('https://doi.org/10.1038/nature12373');
      expect(byUrl, isNotNull);

      // 5. Retrieve by Title
      final byTitle = HiveService.getCachedGraph('Quantum Teleportation Across 143 km');
