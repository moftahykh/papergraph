...
void main() {
  ...
  group('Critical Fix 1 & 2: HiveService Bidirectional Sync Tests', () {
    test(
      'Saving a PaperModel automatically syncs to CanonicalPaper box',
      () async {
        ...
        expect(
          canonicalList.any((p) => p.canonicalId == 'sync-paper-1'),
          isTrue,
        );
        final retrievedCanonical = canonicalList.firstWhere(
          (p) => p.canonicalId == 'sync-paper-1',
        );
    ...
    test(
      'Saving a CanonicalPaper automatically syncs to Favorites box',
      () async {
        ...
        final retrievedModel = favList.firstWhere(
          (p) => p.id == 'sync-canonical-2',
        );
        expect(
          retrievedModel.title,
